...
DELETE FROM `binhluan` WHERE `san_pham_id` IN (SELECT `id` FROM `sanpham`);

-- Clear cart items for old products
DELETE FROM `chi_tiet_gio_hang` WHERE `san_pham_id` IN (SELECT `id` FROM `sanpham`);

-- Clear order details for old products  
DELETE FROM `chi_tiet_don_hang` WHERE `san_pham_id` IN (SELECT `id` FROM `sanpham`); 